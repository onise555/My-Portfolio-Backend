...
namespace Portfolio.Asp.Services
{
    public class S3Service
    {
        ...
        public S3Service(IConfiguration config)
        {
            ...
            // რადგან სახელები გაასწორე, აქ ჩაჯდება შენი ერთიანი სახელი (მაგ. images-55)
            _bucketName = config["AWS_S3_BUCKET_NAME"] ?? config["S3Config:BucketName"] ?? "";

            // ვიყენებთ მუშა ენდპოინტს
        ...
        public async Task<string?> UploadFileAsync(IFormFile? file, string folder = "users/images")
        {
            ...
            var putRequest = new PutObjectRequest
            {
                ...
                CannedACL = S3CannedACL.PublicRead, // ხსნის წვდომას ინტერნეტისთვის
                DisablePayloadSigning = true
            };

            await _s3Client.PutObjectAsync(putRequest);

            // აწყობს ლინკს: https://bucket.endpoint/key
